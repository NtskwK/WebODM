...
class YcMonitorFilter(filters.FilterSet):
    class Meta:
        model = MonitorPoint
        fields = ['id', 'name', 'project_id', 'category_id', 'is_delete']
...
class YcMonitorCategoryFilter(filters.FilterSet):
    class Meta:
        ...
        fields = ['id', 'remark', 'enable', 'monitor_date_last', 'is_delete']
...
class YcMonitorCategoryViewSet(viewsets.ModelViewSet):
    ...
    def list(self, request, *args, **kwargs):
        queryset = self.filter_queryset(
            self.get_queryset()).filter(~Q(is_delete=True))
