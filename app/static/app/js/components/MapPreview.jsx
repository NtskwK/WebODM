...
class MapPreview extends React.Component {
  ...
  componentDidMount() {
    ...
    this.map.fitBounds([
      [13.772919746115805, 45.664640939831735],
      [13.772825784981254, 45.664591558975154],
    ]);
  ...
  loadNewFiles = () => {
    ...
    this.readExifData()
      .then(() => {
        let images = this.sampled(this.exifData, this.MaxImagesPlot).map(
          (exif) => {
            ...
            if (this.hasTimestamp)
              layer.feature.properties["Timestamp"] = exif.timestamp;
            return layer;
          }
        );
        ...
        if (this.hasTimestamp) {
          let coords = this.exifData.map((exif) => [
            exif.gps.latitude,
            exif.gps.longitude,
          ]);
          ...
          .then((exif) => {
            ...
            this.exifData.push({
              ...
              gps: {
                ...
                altitude:
                  exif.GPSAltitude !== undefined ? exif.GPSAltitude : null,
              },
              timestamp,
            });
  ...
  render() {
    return (
      <div style={{ height: "280px" }} className="map-preview">
        ...
        {this.state.error === "" &&
        this.exifData.length > this.MaxImagesPlot ? (
          <div
            className="plot-warning btn-warning"
            title={interpolate(
              _("For performance reasons, only %(num)s images are plotted"),
              { num: this.MaxImagesPlot }
            )}
          >
            ...
            <ul className="dropdown-menu">
              <li>
                <a
                  ...
                  onClick={() => this.download("geojson")}
                >
                  <i className="fas fa-map fa-fw"></i> GeoJSON
                </a>
                <a
                  href="javascript:void(0);"
                  onClick={() => this.download("csv")}
                >
                  <i className="fas fa-file-alt fa-fw"></i> CSV
                </a>
              </li>
            </ul>
          </div>
        ...
        {this.state.error === "" ? (
          <div className="crop-control">
            <button
              ref={(domNode) => {
                this.cropButton = domNode;
              }}
              type="button"
              onClick={this.toggleCrop}
              className={
                "btn btn-sm " +
                (this.state.cropping ? "btn-default" : "btn-secondary")
              }
