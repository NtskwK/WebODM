...
import MonitorDataList from "./MonitorDataList";
...
class MonitorListItem extends React.Component {
  static propTypes = {
    ...
    data: PropTypes.object.isRequired, // monitor json
  ...
  handleTaskSaved(taskInfo) {
    ...
    $.ajax({
      ...
      .fail(() => {
        this.setState({ error: _("创建失败，请稍后重试……") });
        this.handleTaskCanceled();
      });
  }
  ...
  render() {
    ...
    let deleteWarning = _(
      "与此任务相关的所有信息，包括图像、地图和模型将被删除。你确定吗?"
    );
    if (!data.owned)
      deleteWarning = _(
      ...
      >
        ...
        <div className="row no-margin">
          ...
          <div className="row project-links">
            {/* {numTasks > 0 ? 
              <span>
                ...
                <a href="javascript:void(0);" onClick={this.toggleTaskList}>
                  {interpolate("%(count)s Tasks", { count: numTasks})} <i className={'fa fa-caret-' + (this.state.showTaskList ? 'down' : 'right')}></i>
                </a>
              </span>
              : ""} */}
            ...
            <i key="edit-icon" className="fas fa-chart-line"></i>
            <a
              key="edit-text"
              href="javascript:void(0);"
